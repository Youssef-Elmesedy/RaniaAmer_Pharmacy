...
{
    // Assigned by the database (identity column) — a short, human-friendly reference
    // separate from the Guid Id, shown to customers and staff.
    ...
    public void AddItem(Guid productId, string productName, decimal unitPrice, string saleUnitName, decimal quantity, string? note)
    {
        _items.Add(OrderItem.Create(Id, productId, productName, unitPrice, saleUnitName, quantity, note));
    ...
    // Only a Pending order can be cancelled — once it's Completed (prepared/handed over,
    // possibly with a credit balance already recorded against the customer), cancelling it
    // would silently corrupt that history. Handle a completed order's return/refund manually
    // instead.
    public void Cancel()
    {
        ...
        if (Status != OrderStatus.Pending)
            throw new BusinessException("لا يمكن إلغاء طلب تم تجهيزه بالفعل.", nameof(Status));
